/* Соглашение: строка названий тканей хранится в отдельном векторе и используется тогда, когда нужно; также не используем просто стринг-структуру*/
#include <iostream>
#include <fstream>
#include <sstream>
#include <math.h>
...
#define DET ';' //разделитель
#define DELBN '/'
#define LEN 10 //количество параметров, который не относятся к тканям
#define MAXFILENAMELEN 100
#define GNAME 1 //какая колонка отвечает за имя гена - неуникальное, каноническое
#define PROBE 1 //какая колонка отвечает за номер пробы - уникальное
#define TITLE 1 //pr.paramID[0] отвечает за проверку probe ID на равенство
...
{	
	...
	int M = 0, N = 0; //количество строк и столбцов в файле
...
/**************** Заполнение мапа парами <имя_гена, проба> ****************/
	int chromnum;
	...
	while(getline(sdata, s)) 
	{
		...
		allnames.push_back(gname); //собрать все имена
...
DOPF:
	...
	{
//Вставка столбцов с данными в мапу 
...
//		L++;
		...
		//на этом этапе есть вектор с названиями столбцов
		vector <int> numbofcol(L); //знаем - считаем с командной строки
	...
	/*	numbofcol.at(3) = 1;
		...
		vector <vector<string>> insdatavec; //размера = количество проб
	...
	//		cout<<sins<<endl;
		...
		//Считали строку с заголовками
	//при этом знаем номера необходимых столбцов: insdataname[i] - i и есть номер стобца
	//теперь нужно повставлять эти значения в мапу
	...
	for (multimap<string, numProbe>::iterator iter = genemap.begin(); iter != genemap.end(); ++iter)
		...
		//		cout<<k<<endl;
			...
			//	cout<<TMP0.size()<<endl;
				if(TMP0[TITLE]==pr.paramsID[PROBE]) // считаем, что это уникальные имена
				{
	...
	//	MMapStr_numPrintF(insertMap, swaptissues); /// не печатат ------------------------
		goto start;
	}
	else if(answer == 8) // удалить дубли
	{
			int choic;
	CHOICE:
			...
			allnames.swap(allnames2);  // удалили повторяющиеся имена из списка имен
			for(int itt = 0; itt<allnames.size(); itt++)
			{	
				if(genemap.size() == 0) 
					break;
